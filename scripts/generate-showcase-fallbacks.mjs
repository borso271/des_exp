...
import {fitEllipse} from '../showcase/renderers/ellipse.js';
for(const p of presets.filter(p=>p.renderer!=='original'&&p.renderer!=='image')){
 ...
 if(p.id==='squares'||p.id==='moving'){
  ...
  const g=fitEllipse(p.geometry,w,h);
 ...
 const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}"><title>${p.name}</title>${body}</svg>\n`;
