// Derived from js/light-columns/moving-geometry.js. Refresh with scripts/sync-showcase-renderers.mjs.
// Rendering only; no prototype UI or application bootstrap.
import {randomAt} from './geometry.js';
const clamp = (value, low, high) => Math.max(low, Math.min(high, value));
  ...
  function build(settings, width, height) {
    ...
    const sites = Array.from({ length: count }, (_, index) => {
      const r = n => randomAt(settings.seed, index * 16 + n);
